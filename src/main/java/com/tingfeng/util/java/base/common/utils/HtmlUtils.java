...
public class HtmlUtils {  
	 ...
	 * @return
	 */
    public static String getHtmlByTransString(String str){  
        if (str == null)  
            return null;  
    ...
    /**
	 * 过滤用户输入的URL地址（防治用户广告） 目前只针对以http或www开头的URL地址 本方法调用的正则表达式，
	 ...
	 * @return 结果串
	 */
	public static String getStringByFomateToFullForm(String str, String pt) {
		String regEx = "<" + pt + "\\s+([\\S&&[^<>]]*)/>";
    ...
    public static String transUrl2(String prefex,String s){
      return s.replaceAll("/ASPPV1/", s+"/ASPPV1/");
    }
    /**
     ...
     * @return
     */
    public static String transUrl(String prefex,String s){
    	try{
    	...
    	{ 	String tmp=matcher.group(1);
    		...
    		h=h.replace(tmp,prefex+tmp);  
    		has=true;
    	}
    	
    	 pattern = Pattern.compile(".*href=\\\\*\\\"(?!http://)([^\\\\][^\\s]+[^\\\\=])\\\\*\\\".*");
    	...
    	pattern = Pattern.compile(".*url=\\\\*\\\"(?!http://)([^\\\\][^\\s]+[^\\\\=])\\\\*\\\".*");
	   	 matcher = pattern.matcher(s); 
	   	while(matcher.find())
	   	{ 	String tmp=matcher.group(1);
	   		//System.out.println(tmp);
