...
public class WebServiceUtils {
    ...
    /**
     * 当进度编号大于updateCallSpace%的时候,调用回调函数,默认0.5%
     */
    public static double updateCallSpace = 0.5;
    ...
    private static String buildRequestData(String namespace, String methodName,
        ...
        return StringUtils.doAppend(soapRequestData ->{
            ...
            if (params != null) {
                ...
                for (String name : nameSet) {
                    ...
                    paramNames.append("<ns:" + name + ">" + value + "</ns:" + name + ">");
                }
            }
            if (!hashParams) {
                soapRequestData.append("<ns:" + methodName + "/>");
            } else {
                soapRequestData.append("<ns:" + methodName + ">" + paramNames + "</ns:" + methodName + ">");
            }
        ...
        try {
            ...
            if (fin != null) {
                ...
                while (fin.read(buffer, 0, bufferSize) != -1) {
                    ...
                    if (callBack != null && percent - lastPercent >= updateCallSpace) {
     ...
     * @param namespace
     * @param methodName
     * @param httpParams 发送的内容的键值对
     * @param tryCount   失败后重新尝试的次数
     * @return
     */
