...
public class RegExpUtilsTest {
    ...
    public void testPhoneCNPattern() {
        Assert.assertTrue("13812345678应该是中国手机号", 
            RegExpUtils.isMatch("13812345678", RegExpUtils.PatternStr.phoneCN, true));
        Assert.assertTrue("15912345678应该是中国手机号", 
            RegExpUtils.isMatch("15912345678", RegExpUtils.PatternStr.phoneCN, true));
        Assert.assertTrue("18612345678应该是中国手机号", 
            RegExpUtils.isMatch("18612345678", RegExpUtils.PatternStr.phoneCN, true));
        Assert.assertTrue("16612345678应该是中国手机号", 
            RegExpUtils.isMatch("16612345678", RegExpUtils.PatternStr.phoneCN, true));
        Assert.assertFalse("12312345678不应该是中国手机号（号段不对）", 
            RegExpUtils.isMatch("12312345678", RegExpUtils.PatternStr.phoneCN, true));
        Assert.assertFalse("1381234567不应该是中国手机号（位数不对）", 
            RegExpUtils.isMatch("1381234567", RegExpUtils.PatternStr.phoneCN, true));
    }
    ...
    public void testIpV4Pattern() {
        Assert.assertTrue("192.168.1.1应该是IPv4地址", 
            RegExpUtils.isMatch("192.168.1.1", RegExpUtils.PatternStr.ipV4, true));
        Assert.assertTrue("0.0.0.0应该是IPv4地址", 
            RegExpUtils.isMatch("0.0.0.0", RegExpUtils.PatternStr.ipV4, true));
        Assert.assertTrue("255.255.255.255应该是IPv4地址", 
            RegExpUtils.isMatch("255.255.255.255", RegExpUtils.PatternStr.ipV4, true));
        Assert.assertFalse("256.168.1.1不应该是IPv4地址（超过255）", 
            RegExpUtils.isMatch("256.168.1.1", RegExpUtils.PatternStr.ipV4, true));
        Assert.assertFalse("192.168.1不应该是IPv4地址（缺少一段）", 
            RegExpUtils.isMatch("192.168.1", RegExpUtils.PatternStr.ipV4, true));
    }
    ...
    public void testIntOrFloatNumberPattern() {
        Assert.assertTrue("123应该是整数或浮点数", 
            RegExpUtils.isMatch("123", RegExpUtils.PatternStr.intOrFloatNumber, true));
        Assert.assertTrue("123.45应该是整数或浮点数", 
            RegExpUtils.isMatch("123.45", RegExpUtils.PatternStr.intOrFloatNumber, true));
        Assert.assertTrue("-123应该是整数或浮点数", 
            RegExpUtils.isMatch("-123", RegExpUtils.PatternStr.intOrFloatNumber, true));
        Assert.assertTrue("+123.45应该是整数或浮点数", 
            RegExpUtils.isMatch("+123.45", RegExpUtils.PatternStr.intOrFloatNumber, true));
        Assert.assertTrue("0.45应该是整数或浮点数", 
            RegExpUtils.isMatch("0.45", RegExpUtils.PatternStr.intOrFloatNumber, true));
    }
    ...
    public void testNotIntegerPattern() {
        String result = "abc123def".replaceAll(RegExpUtils.PatternStr.notInteger, "");
